...
namespace Unity.Animations.SpringBones
{
    ...
    public class SpringBoneInspector : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("피벗 선택", SpringBoneGUIStyles.ButtonStyle))
            {
            ...
            {
                EditorGUILayout.ObjectField("매니저", managers[managerIndex], typeof(SpringManager), true);
            }
            var newEnabled = EditorGUILayout.Toggle("활성화", bone.enabled);
            ...
            if (newEnabled != bone.enabled)
            {
                ...
                if (targetBones.Any())
                {
                    Undo.RecordObjects(targetBones.ToArray(), "SpringBone 활성 상태 변경");
            ...
            showOriginalInspector = EditorGUILayout.Toggle("기본 인스펙터 표시", showOriginalInspector);
            GUILayout.Space(Spacing);
        ...
        private void RenderAngleLimitVisualization()
        {
            ...
            GUILayout.Label("Y 축 각도 제한");
            GUILayout.Label("Z 축 각도 제한");
        ...
        private void OnEnable()
        {
            ...
            var forceProperties = new PropertyInfo[] {
                new PropertyInfo("stiffnessForce", "강성"),
                new PropertyInfo("dragForce", "공기 저항"),
                new PropertyInfo("springForce", "중력"),
                new PropertyInfo("windInfluence", "바람 영향")
            };

            var angleLimitProperties = new PropertyInfo[] {
                new PropertyInfo("pivotNode", "피벗"),
                new PropertyInfo("angularStiffness", "회전 강성"),
                new AngleLimitPropertyInfo("yAngleLimits", "Y 축 각도 제한"),
                new AngleLimitPropertyInfo("zAngleLimits", "Z 축 각도 제한")
            };

            var lengthLimitProperties = new PropertyInfo[] {
                new PropertyInfo("lengthLimitTargets", "타겟")
            };

            var collisionProperties = new PropertyInfo[] {
                new PropertyInfo("radius", "반지름"),
                new PropertyInfo("sphereColliders", "구체"),
                new PropertyInfo("capsuleColliders", "캡슐"),
                new PropertyInfo("panelColliders", "패널")
            };

            propertySets = new PropertySet[] {
                new PropertySet("힘", forceProperties),
                new PropertySet("각도 제한", angleLimitProperties),
                new PropertySet("거리 제한", lengthLimitProperties),
                new PropertySet("충돌", collisionProperties),
            };
