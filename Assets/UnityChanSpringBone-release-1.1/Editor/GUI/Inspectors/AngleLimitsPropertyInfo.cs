...
namespace Unity.Animations.SpringBones
{
    namespace Inspector
    {
        public class AngleLimitPropertyInfo : PropertyInfo
        {
            ...
            {
                minSlider = new FloatSlider("최소값", 0f, -180f);
                maxSlider = new FloatSlider("최대값", 0f, 180f);
            }
            ...
            public override void Show()
            {
                ...
                if (minProperty != null && maxProperty != null)
                {
                    ...
                    updateValuesTogether = GUILayout.Toggle(updateValuesTogether, "동시에 변경");
                    ...
                    if (GUILayout.Button("최소값 기준 통일"))
                    {
                        maxProperty.floatValue = -minProperty.floatValue;
                    }

                    if (GUILayout.Button("최대값 기준 통일"))
                    {
                        minProperty.floatValue = -maxProperty.floatValue;
                    }

                    if (GUILayout.Button("반전"))
                    {
